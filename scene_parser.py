...
def make_mat4(array: list):
    if len(array) != 16:
        print("Invalid matrix array size:", len(array), "expected 16")
        return glm.mat4(1.0)
    return glm.mat4(array[0:4], array[4:8], array[8:12], array[12:16])
...
def load_scene(infile: str):
    ...
    cam_fl = data["camera"].get("focalLength", 1.0)
    cam_ap = data["camera"].get("aperture", 0.0)
    cam_dofSamples = data["camera"].get("DOF_Saples", 0.0)
    ...
    for material in data["materials"]:
        ...
        mat_reflection = make_vec3(material.get("reflection", [0, 0, 0]))
        mat_shininess = 0 if "shininess" not in material else material["shininess"]
        mat_ior = material.get("ior", 1.0)
        mat_transparency = material.get("transparency", 0.0)
        mat_hasBumpMap = material.get("hasBumpMap", False)
        material_by_name[mat_name] = hc.Material(mat_name, mat_diffuse, mat_specular, mat_shininess, mat_reflection, mat_ior, mat_transparency,mat_hasBumpMap)
    ...
    return scene.Scene(width, height, jitter, samples,  # General settings
                cam_pos, cam_lookat, cam_up, cam_fov, cam_fl, cam_ap, cam_dofSamples,  # Camera settings
...
def load_geometry( geometry, material_by_name, geometry_by_name ):
    ...
    if g_type == "sphere":
        ...
        g_M = make_mat4(geometry["matrix"])
        return geom.Quadradic(g_name, g_type, g_mats, g_M)
    else:
